...
const Pedido = ({pedido}) => {
    ...
    const [ eliminarPedido ] = useMutation(ELIMINAR_PEDIDO, {
        update(cache) {
            ...
            cache.writeQuery({
                ...
                data: {
                    obtenerPedidosVendedor: obtenerPedidosVendedor.filter( clienteActual => clienteActual.id !== id)
                }
            })            
        }
    });
    ...
    return ( 
        <div className={` ${clase} border-t-4 mt-4 bg-white rounded p-6 md:grid md:grid-cols-2 md:gad-4 shadow-lg`}>
            <div>
                ...
                <select
                    className="mt-2 appearance-none bg-blue-600 border border-blue-800 text-white p-2 text-center rounded leading-tight focus:outline-none focus:bg-blue-600 focus:border-blue-500 uppercase text-xs font-bold"
                    value={estadoPedido}
